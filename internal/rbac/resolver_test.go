...
// TestResolverRPCAPIKeyHeaderPropagation verifies that the per-group
// rpc_api_key_header field flows from the GroupAccess row through to the
// resolved EffectivePermissions, including across multiple memberships
// (header travels with the API key — first non-empty key wins).
func TestResolverRPCAPIKeyHeaderPropagation(t *testing.T) {
	t.Run("single membership propagates header", func(t *testing.T) {
		store := NewMockStore()
		resolver := NewResolver(store, 5*time.Minute)

		store.organizations["org1"] = &Organization{ID: "org1", Slug: "o"}
		group := &Group{ID: "g", OrgID: "org1", Slug: "g", Path: "g"}
		store.groups["g"] = group

		key := "sk-team-a"
		store.groupAccess["g"] = &GroupAccess{
			GroupID:         "g",
			AllowedMethods:  []string{"eth_call"},
			RPCAPIKey:       &key,
			RPCAPIKeyHeader: "X-API-Key",
		}
		store.groupsByOrg["u:org1"] = []*MembershipWithDetails{
			{Membership: &UserMembership{UserID: "u", GroupID: "g"}, Group: group},
		}

		perms, err := resolver.ResolvePermissions(context.Background(), "u", "org1")
		if err != nil {
			t.Fatalf("ResolvePermissions: %v", err)
		}
		if perms.RPCAPIKey != "sk-team-a" {
			t.Errorf("RPCAPIKey = %q, want %q", perms.RPCAPIKey, "sk-team-a")
		}
		if perms.RPCAPIKeyHeader != "X-API-Key" {
			t.Errorf("RPCAPIKeyHeader = %q, want %q", perms.RPCAPIKeyHeader, "X-API-Key")
		}
	})

	t.Run("multi membership: header travels with first non-empty key", func(t *testing.T) {
		store := NewMockStore()
		resolver := NewResolver(store, 5*time.Minute)

		store.organizations["org1"] = &Organization{ID: "org1", Slug: "o"}
		gA := &Group{ID: "gA", OrgID: "org1", Slug: "gA", Path: "gA"}
		gB := &Group{ID: "gB", OrgID: "org1", Slug: "gB", Path: "gB"}
		store.groups["gA"] = gA
		store.groups["gB"] = gB

		// gA has no key; gB has a key with custom header.
		store.groupAccess["gA"] = &GroupAccess{
			GroupID:        "gA",
			AllowedMethods: []string{"eth_call"},
		}
		keyB := "sk-team-b"
		store.groupAccess["gB"] = &GroupAccess{
			GroupID:         "gB",
			AllowedMethods:  []string{"eth_call"},
			RPCAPIKey:       &keyB,
			RPCAPIKeyHeader: "X-API-Key",
		...
		store.groupsByOrg["u:org1"] = []*MembershipWithDetails{
			{Membership: &UserMembership{UserID: "u", GroupID: "gA"}, Group: gA},
			{Membership: &UserMembership{UserID: "u", GroupID: "gB"}, Group: gB},
		}
		...
		if perms.RPCAPIKey != "sk-team-b" {
			t.Errorf("RPCAPIKey = %q, want gB key", perms.RPCAPIKey)
		}
		if perms.RPCAPIKeyHeader != "X-API-Key" {
			t.Errorf("RPCAPIKeyHeader = %q, want %q", perms.RPCAPIKeyHeader, "X-API-Key")
		}
	...
	t.Run("no key configured leaves header empty (fallback to config default)", func(t *testing.T) {
		store := NewMockStore()
		resolver := NewResolver(store, 5*time.Minute)
		...
		if perms.RPCAPIKey != "" {
			t.Errorf("RPCAPIKey = %q, want empty", perms.RPCAPIKey)
		}
		if perms.RPCAPIKeyHeader != "" {
			t.Errorf("RPCAPIKeyHeader = %q, want empty (fallback)", perms.RPCAPIKeyHeader)
		}
	})
}
