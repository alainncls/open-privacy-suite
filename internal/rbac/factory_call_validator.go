...
// It ensures that the creation bytecode passes deployment validation (no unsafe calls).
// The target address is validated at execution time via runtime tracing (debug_traceCall).
...
func (v *FactoryCallValidator) ValidateFactoryCall(
	...
	if len(creationBytecode) > 0 {
		...
		// CREATE/CREATE2 in creation bytecode are allowed — runtime tracing
		// validates the actual execution and auto-registers created addresses.
	}
