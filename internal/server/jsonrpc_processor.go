...
func (p *JSONRPCProcessor) processDebugTrace(ctx context.Context, req *ProcessRequest) *ProcessResult {
	...
	if traceErr != nil {
		p.logAccess(ctx, req, http.StatusForbidden)
		// Opaque client message; the raw upstream node error (which can echo
		// internal addresses, revert reasons, or node config) stays in slog.
		// Matches the file's KD-3 opaque-constant convention. (RD-1178 #9 / RD-934)
		slog.Warn("trace execution failed", slog.String("method", req.Method), slog.String("user", req.UserID), slog.Any("err", traceErr))
