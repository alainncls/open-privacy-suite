...
- Contract grants authorize functions all-or-nothing (`ContractAccess.Functions`,
  ...
  *record*. the unrelated-caller scenario is the concrete case: any `payment` group member may
  read any `getPaymentInfo(paymentId)`.
- RD-1144 tried to protect returns with address-visibility field redaction →
  RD-1188 measured it blanking the record for its own payer/payee (own-record
...
6. **Processor e2e** (mockauth/e2e harness, real server path): Workflow shape —
   provision policy via admin API (super-admin), send create/complete with
   visibleTo through the proxy, assert the end-to-end matrix incl. **own-record regression
   (parties read own record unredacted, RD-1188)** and **the unrelated-caller case (unrelated with a
