...
WorkflowCore is the motivating example only. Grounded in
`docs/workflow-policy-resolver-handoff.md` + the rules spec rules 71/72.
...
Example (Workflow, generic fixture in tests):
...
- **Non-indexed key in event data** (Workflow's `msgId` is in the event data): decode
...
*because* the contract's event baseline is configured deny-by-default (Workflow's config —
...
## 6. Rule 71 / 72 verdict (against the rules spec)
...
- **Rule 71 (events per workflowId): FULLY resolved** by this design, conditional on:
  ...
  record audience (§5) — Workflow's config; (c) explorer symmetry wired (§3). Workflow's
...
(a scalar/`visibleTo` capture covers Workflow today — added only if a client wants the
