...
- **Allowlist of methods that go through eth_call tracing**: the current set is `eth_call` only. Should `eth_call` aliases (chain-specific equivalents under RD-911 prefix wildcards) be traced too? Default: yes, via `ResolveMethodAlias` already pointing them to `eth_call`. Confirm in implementation.
