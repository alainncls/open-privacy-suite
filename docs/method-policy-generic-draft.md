...
async reconciler (same one that lands visibleTo):
  ...
  key       = "payment-123"
  payer     = sender DID        (did:workflow:debtor)
  payee     = param(1)          (0xBb…)
  audience  = visibleTo DIDs    ([did:workflow:settlement])
...
### Example 1 — Workflow `getPaymentInfo` (the policy above)
...
```json
{
  ...
  "visibleTo": ["did:workflow:settlement"],
  "id": 1
}
```
...
(PaymentRegistry, "payment", "payment-123") ->
  payer    = did:workflow:debtor        (sender)
  payee    = 0xBb…                     (param 1)
  audience = [did:workflow:settlement]  (visibleTo)
```
...
1. **Phase 1** — policy engine + the return resolver only (= the original
   spec; closes the Workflow `getPaymentInfo` case).
