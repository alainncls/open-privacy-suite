...
// TestMethodPolicy_Workflow_E2E is the definitive RD-1206 test: a real
// PaymentRegistry on anvil, a real method policy, real signed sends through the
// proxy, and the reconciler promoting captures — asserting the the end-to-end scenario
// matrix (payer/payee/settlement read, unrelated denied), parameter-bound.
func TestMethodPolicy_Workflow_E2E(t *testing.T) {
	...
	require.NoError(t, database.CreateOrganization(ctx, &rbac.Organization{ID: orgID, Slug: "wf-" + orgID[:8], Name: "Workflow", Settings: map[string]any{}}))
	...
	require.NoError(t, database.UpdateContractMethodPolicies(ctx, contract.ID, []byte(workflowMethodPolicy)))
