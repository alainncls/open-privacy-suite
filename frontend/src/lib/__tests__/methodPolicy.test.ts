...
const workflowWizard: WizardState = {
  recordType: "payment",
...
describe("compileMethodPolicy", () => {
  it("compiles the Workflow policy to the exact backend schema", () => {
    const doc = compileMethodPolicy(workflowWizard);
    expect(doc).toEqual({
      records: {
        payment: {
          capture: [
  ...
  it("merges into an existing document, preserving other record types", () => {
    ...
    const doc = compileMethodPolicy(workflowWizard, existing);
  ...
  it("omits the return rule when no return paths are chosen (capture-only)", () => {
    const doc = compileMethodPolicy({ ...workflowWizard, returnPaths: [] });
...
describe("validateWizard", () => {
  const fns = parseAbiFunctions(paymentABI);
  it("passes the coherent Workflow state", () => {
    expect(validateWizard(workflowWizard, fns)).toBeNull();
  });
  it("rejects a key-type mismatch between writer and reader", () => {
    ...
    const bad = { ...workflowWizard, writerKeyIndex: 1 };
    expect(validateWizard(bad, fns)).toMatch(/key types must match/);
  });
  it("requires at least one allow rule", () => {
    expect(validateWizard({ ...workflowWizard, allowFields: [], returnPaths: [] }, fns)).toMatch(/allow rule/);
  });
  it("requires a param index for param-sourced fields", () => {
    const bad = { ...workflowWizard, remember: [{ field: "payee", source: "param" as const, merge: "set_once" as const }] };
    expect(validateWizard(bad, fns)).toMatch(/pick which one/);
  });
  it("rejects an allow field that isn't captured", () => {
    expect(validateWizard({ ...workflowWizard, allowFields: ["ghost"] }, fns)).toMatch(/isn't a captured field/);
  });
  ...
  it("rejects a return path that isn't an address output of the reader (H3)", () => {
    expect(validateWizard({ ...workflowWizard, returnPaths: ["amount"] }, fns)).toMatch(/not an address output/);
  });
});
