...
// Full Workflow record: two captures (create + complete) and one reader with a
// captured-field rule AND a return rule.
const workflow: WizardState = {
  records: [{
    recordType: "payment",
    captures: [
...
describe("compileWizard", () => {
  it("compiles the full Workflow policy (2 captures, callerIn+return) to exact schema", () => {
    expect(compileWizard(workflow)).toEqual({
      records: {
        payment: {
          capture: [
...
describe("validateWizard", () => {
  it("passes the full Workflow policy", () => {
    expect(validateWizard(workflow, fns)).toBeNull();
  });
  it("rejects visibleTo + set_once", () => {
    const s = structuredClone(workflow);
  ...
  it("rejects duplicate record types", () => {
    const s: WizardState = { records: [workflow.records[0], structuredClone(workflow.records[0])] };
...
describe("decompileWizard round-trip", () => {
  it("compile → decompile → compile is stable for the Workflow policy", () => {
    const doc = compileWizard(workflow);
...
const workflowFull: WizardState = {
  records: [{
    recordType: "payment",
    captures: [
      { writerSig: "createPayment(string,address,uint256)", keyIndex: 0, remember: [
...
describe("compileWizard — events + transactions", () => {
  it("emits the EXACT locked schema (deep-equal)", () => {
    const doc = compileWizard(workflowFull);
  ...
  it("omits events/transactions keys entirely when empty (matches Go omitempty)", () => {
    const s: WizardState = { records: [{
      recordType: "payment",
      captures: workflowFull.records[0].captures,
      readers: workflowFull.records[0].readers,
      events: [],
      transactions: [],
    }]};
  ...
  it("carries a literal principal + where into the audience allow rule", () => {
    const s = structuredClone(workflowFull);
...
describe("validateWizard — events + transactions", () => {
  it("passes the full record", () => {
    expect(validateWizard(workflowFull, fns, events)).toBeNull();
  });
