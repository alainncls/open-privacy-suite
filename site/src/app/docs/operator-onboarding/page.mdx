...
<Callout variant="info" title="Tx-sending groups need the EIP-1559 fee methods">
Any group whose users **send transactions** with a wallet or CLI (MetaMask, `cast`, ethers, viem) also needs `eth_feeHistory`, `eth_maxPriorityFeePerGas`, and `eth_getBlockByNumber` in its allowlist: those clients build EIP-1559 transactions by default and call the fee-estimation methods *before* signing, so without them the send fails before it ever reaches the chain. The deployer example below includes them. (Alternatively the client can send legacy transactions — e.g. `cast send --legacy` — which price gas from `eth_gasPrice` and skip the fee methods.)
</Callout>
...
The `/access` body also accepts optional `rate_limit_rps`, `rate_limit_daily`, and a per-group upstream `rpc_api_key` — see [RBAC → Per-Group RPC API Keys](/docs/rbac#per-group-rpc-api-keys).
