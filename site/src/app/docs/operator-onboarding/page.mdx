...
Run these as the **tier-2 org admin** from Part 1, using their session JWT (`Authorization: Bearer <their-JWT>`) — that's the intended day-to-day path (least privilege, and the audit log attributes each change to that admin). A super-admin `X-Admin-Token` *can* technically run them all too, but **reserve it** for the Part 1 bootstrap, creating/updating orgs, minting org admins, and the maintenance endpoints. The examples below keep the `X-Admin-Token` header only to stay copy-paste-consistent with Part 1 — swap in `Authorization: Bearer <JWT>` for normal day-to-day use.
...
All paths are prefixed `/api/v1/admin`. All require `X-Admin-Token` (super-admin) or `Authorization: Bearer <JWT>` (tier-2 org admin), and all are gated to private-network source IPs (token + the IP check described in [Before you start](#before-you-start)).
