...
graph TD
    ...
    IDX["chain-indexer<br/><small>Blocks, Txs, Stats</small>"]
    Eth["Ethereum Node<br/><small>Erigon / Anvil</small>"]

    User --> BE
    BE -->|"Proxy Mode (REST)"| Proxy
    BE -->|"Standalone Mode (INDEXER_URL)"| IDX
    Proxy --> IDX
    Proxy --> Eth
    IDX -->|polls| Eth
...
In this mode, the explorer reads chain data from the **chain-indexer** (set `INDEXER_URL`), with its own PostgreSQL database as a limited direct fallback. This is suitable for development or deployments where centralized privacy enforcement is not required. (The `privacy` Go build tag compiles the chain-indexer client out of the binary entirely, so a privacy-mode build cannot use this mode.)
...
|----------|-------------|
| `GET /api/v1/explorer/blocks` | List recent blocks with pagination |
| `GET /api/v1/explorer/blocks/:number` | Get detailed block information |
| `GET /api/v1/explorer/transactions` | List recent transactions |
| `GET /api/v1/explorer/addresses/:address/stats` | Get address-specific activity stats |
| *(removed)* | The separate shared-logs endpoint has been replaced by `visibleTo` integration into regular views |
| `GET /api/v1/explorer/sync/status` | Get indexer synchronization levels |
...
To enable Proxy Mode in the Block Explorer, set `PRIVACY_PROXY_URL` and leave `INDEXER_URL` unset — the presence of `PRIVACY_PROXY_URL` selects proxy mode (setting both, or neither, is rejected at startup):
...
| `SSO_CLIENT_ID` | OAuth client identifier sent in the authorize request. Defaults to `explorer`. In production, use the per-environment opaque ID listed in the proxy's `OAUTH_FIRST_PARTY_CLIENTS` (e.g. `explorer-prod-${random}`). |
| `SSO_CLIENT_SECRET` | Plaintext client secret sent at `/oauth/token` via HTTP Basic (RFC 6749 `client_secret_basic`). The proxy bcrypt-verifies it against the hash stored under this `SSO_CLIENT_ID` in `OAUTH_FIRST_PARTY_CLIENTS`. Required in production; leave empty in dev only if the proxy's allowlist is also empty. |
...
Stripping the `value` field on mixed-party transactions **does not guarantee financial privacy**. The transferred amount may still be inferable from the public address's balance deltas across adjacent blocks. See [Privacy Requirements](/docs/security/privacy-requirements) for the full analysis.
...
When a transaction is submitted with the `visibleTo` parameter (renamed from `logVisibleTo`), the specified DIDs are stored alongside the transaction hash. Transactions shared via `visibleTo` appear in the regular Transactions and Token Transfers pages for the listed DIDs -- no separate endpoint is needed.

### How it works

1. A transaction sender includes `visibleTo: ["did:example:bob"]` in their transaction metadata.
2. The proxy strips the field and stores the mapping before forwarding the transaction to the node.
3. When Bob views the Transactions or Token Transfers pages, the visibility filter includes these shared tx hashes, so they appear alongside Bob's own transactions.
4. Event logs from these transactions are also visible to Bob via `eth_getLogs` (the `visibleTo` fallback extends param rule checks).

This is purely additive -- it never restricts existing event rule access. If Bob already has access to the logs via RBAC event rules, `visibleTo` simply provides an additional discovery path.
