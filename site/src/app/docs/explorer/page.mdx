...
| `SSO_CLIENT_SECRET` | Plaintext client secret sent at `/oauth/token` via HTTP Basic (RFC 6749 `client_secret_basic`). The proxy bcrypt-verifies it against the hash stored under this `SSO_CLIENT_ID` in `OAUTH_FIRST_PARTY_CLIENTS`. Required in production (RD-1006); leave empty in dev only if the proxy's allowlist is also empty. |
