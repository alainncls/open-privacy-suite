...
graph TD
    ...
    Proxy["Open Privacy Suite<br/><small>Auth → RBAC → Trace → Forward → Redact</small>"]
    Eth["Ethereum Node<br/><small>JSON-RPC execution layer (Erigon)</small>"]
    DB["PostgreSQL<br/><small>Policies, RBAC</small>"]
    Audit["Audit database<br/><small>append-only access log, restricted role</small>"]
    Redis["Redis<br/><small>sessions, cache, RBAC — optional</small>"]
...
Policies and RBAC live in PostgreSQL. The access log is written to a **separate, append-only audit database** with a restricted insert-only role, so the tamper-evident audit trail is isolated from the operational data (see [Audit integrity](/docs/security/audit-integrity)). **Redis** is optional — configure it for any multi-instance deployment, where it holds shared sessions, cache, and RBAC state.
...
10. **Forward** -- Send the validated request with the resolved group or fallback API key. The upstream RPC proxy owns request-rate quotas; the Open Privacy Suite's per-key circuit breaker sheds requests after an upstream `429`.
...
| PostgreSQL | `5432` | Policy storage, RBAC |
| Audit database | `5432` | Append-only access log — separate database + restricted role (may share the server) |
| Redis | `6379` | Sessions, cache, RBAC — optional; required for multi-instance |
| Ethereum Node (Anvil/Erigon) | `8545` | JSON-RPC execution layer |

## Privacy-mode topology

In privacy mode the deployment adds a block-explorer fronted by a BFF (Backend-for-Frontend) and a chain-indexer feeding the BFF. The compose manifest splits the trust surface into two internal Docker networks plus a public one — `proxy-backend` is the only legitimate bridge between them.
...
graph LR
    ...
    PROXY[proxy-backend<br/><small>public + bff-zone + indexer-zone</small>]
    NODE[Ethereum node]
    ...
    AUDIT[(audit database<br/><small>append-only, restricted role</small>)]
...
- **`indexer-zone`** holds the chain-indexer and its postgres. Only `proxy-backend` attaches to this network.
- **`bff-zone`** holds the block-explorer BFF and its postgres. `proxy-backend` and the explorer-frontend (for nginx upstream resolution) attach here.
- **`public`** carries browser ingress only — `proxy-backend` (port `8080`), `proxy-frontend` (port `5173`), and `block-explorer-frontend` (port `3001`).
