...
`ExpandClaims()` normalizes implied claims before saving on the backend. Only three claims gate behavior today; the legacy `read` and `write` claims were removed in RD-853 — non-privileged method access is now the exclusive job of the per-group method allowlist.
...
Methods that don't require `admin`/`deploy`/`upgrade` are gated solely by the allowlist.
...
- **Group** -- permission container scoped to an organization. **Flat** since RD-804 — the `parent_id` and `path` columns persist on the schema but the resolver no longer walks them. Users get UNION of every group they're a member of.
...
Groups are **flat** — a group's effective permissions are exactly its own `GroupAccess` + `ContractGrant` set. There is no parent/child walk, no path-based inheritance, no INTERSECTION across a chain. RD-804 (`b85039f`) removed the resolver's hierarchy traversal; the schema columns (`parent_id`, `path`, `depth`) are kept for back-compat but the runtime ignores them.
...
3. Rate limits across memberships: **MAXIMUM** is honored (the most generous limit applies).
...
- **Claims checkboxes** (RD-872 follow-up) — `deploy` and `upgrade` are now exposed as explicit per-grant checkboxes. They take effect only on this contract for this group; granting `deploy` on a single contract does NOT widen the group's overall deploy claim.
...
<Callout variant="info" title="Default group is system-protected (RD-877)">
...
<ApiEndpoint method="POST" path="/api/users/:id/memberships" description="Assign user to a group with a role (by user UUID)" />
<ApiEndpoint method="POST" path="/api/orgs/:org_id/memberships/by-did" description="Onboard by DID — tier-2 admin pulls a DID into their own org without a super-admin handoff (RD-945)" />

#### Onboarding by DID

`POST /api/v1/admin/orgs/:org_id/memberships/by-did` is the canonical path for a tier-2 org admin to add a user to their own org when they only know the user's DID. Before RD-945 the only way to do this was to ask a super-admin to look up the DID's UUID and pass it back — tier-2 admins were structurally blocked even though the action was within their authority.
...
| `400` | Body missing `did` or `group_id` | `{"error": "invalid request body"}` |
...
| `404` | Banned DID, or `EnsureUserExists` returned no user (defensive) | `{"error": "user not found"}` — never surfaces ban status to the caller |
| `409` | DID is already a member of `group_id` (idempotent repeat) | `{"error": "user is already a member of this group"}` |
| `500` | DB / RBAC controller failure | Generic; details in proxy logs only |
