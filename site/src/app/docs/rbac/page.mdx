...
The Privacy Proxy implements a **multi-tenant, hierarchical RBAC** system that protects blockchain
...
3. **Unregistered contracts use default claims** -- if a contract is not registered to any organization, any authenticated user with matching claims (`read`, `write`, etc.) can access it. A contract registered to a *different* org is always denied regardless of the user's claims.
...
|--------------|-------------------|-----------------|
| Unregistered | Allowed (default claims) | Allowed (default claims) |
...
2. **Unregistered contracts**: only deploy/admin users can access. Read/write-only users must use registered contracts.
...
4. **Deployer auto-grant**: user who deployed a contract gets read+write automatically.
...
| Unregistered contract | User's default org | Allowed (default claims apply — see note below) |
| Contract in Org C (user not member) | -- | Request denied |
| No target (deployment) | User's default org | Use default org memberships |

**Note on "unregistered" contracts:** Contracts deployed through the proxy are **never unregistered** — they are pre-registered to the deployer's org before the transaction is forwarded to the node, closing any race window. "Unregistered" only applies to contracts that exist on-chain but were not deployed through the proxy (e.g., system/genesis contracts, or contracts deployed via direct node console access). These are treated as public for any authenticated user with claims.
...
- `?auto_created=true` — show only auto-created deployer groups
- `?auto_created=false` — show only manually created groups
- `?search=term` — filter by name or slug (case-insensitive)

### Batch Operations

<ApiEndpoint method="POST" path="/api/orgs/:org_id/contracts/batch-move" description="Move contracts from auto-created groups to a target group" />
<ApiEndpoint method="POST" path="/api/orgs/:org_id/groups/batch-delete" description="Delete multiple groups atomically" />
<ApiEndpoint method="POST" path="/api/orgs/:org_id/groups/batch-delete-preview" description="Preview the impact of batch-deleting groups" />

Batch operations are atomic — if any step fails, the entire operation rolls back.
Auto-created groups are generated when deployers deploy contracts and can be identified by the `auto_created` flag.
