...
<ApiEndpoint method="POST" path="/api/users/:id/memberships" description="Assign user to a group with a role (by user UUID; target user must already be in caller's full-admin scope — RD-942)" />
...
#### User-scope gate on `POST /users/:id/memberships` (RD-942)
