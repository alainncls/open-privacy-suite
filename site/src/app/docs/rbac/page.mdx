...
Configured via the `ADMIN_API_TOKEN` environment variable. This is not a user account -- it is a shared secret held by the platform operator, used for **bootstrap and platform-level** operations, not day-to-day tenant management.

Super admin is scoped to:
...
- **Fleet / cluster-wide settings** — shared-infrastructure allowlist, base currency, Azure-tenant allowlist, global sanctions, system runtime toggles, and the system default / anonymous groups.

Super admin **cannot** perform per-org tenant management — creating or editing regular groups, setting regular-group access, registering contracts, creating contract grants, or onboarding members into regular groups all return `403 Forbidden` for the `X-Admin-Token`. Those are the org admin's job (tier-2, below), so that a platform operator who is **not** a member of a tenant org cannot silently reshape that org's access. (The system default org/group are exempt — they are platform infrastructure, not a tenant.)
...
- A "View as user" dashboard mode (continuous browse-as) is not part of this release.
