...
<Callout type="warning" title="Cross-organization authorization oracle">
The cross-org oracle is disabled by default. Enabling it requires a dedicated
`CROSS_ORG_AUTHORIZATION_ORACLE_TOKEN`, distinct from admin/operator tokens,
and an explicit `CROSS_ORG_AUTHORIZATION_ORACLE_ORG_IDS` allowlist. Calls use
`POST /api/v1/admin/cross-org-authorization-oracle` and the
`X-Cross-Org-Authorization-Oracle-Token` header. Even `verdict_only` reveals
policy information and must be treated as a cross-tenant trust boundary.
</Callout>
...
`POST /api/v1/admin/policy-check` lets a trusted service test a JSON-RPC operation before submission.
Use the full `ADMIN_API_TOKEN`. The operator token and JWT admin credentials cannot use this endpoint.

The request identifies one subject by DID or linked Ethereum address. An optional `org_id` selects the organization.
A supplied `from` address must link to the subject. The response contains only `allowed` and a safe reason category.

The endpoint checks RBAC and nested EVM calls with `debug_traceCall`. Write operations also use a side-effect-free compliance preview.
The preview does not send a transaction, write a compliance log, or consume a travel-rule record.

The upstream node must support `debug_traceCall`. The endpoint fails closed when tracing is unavailable or exceeds five seconds.
Trace checks use the same upstream RPC credential as live calls. The service credential is limited to 100 traces per second and 10,000 traces per day.
`debug_traceCall` and `debug_traceTransaction` are not supported. The endpoint also uses the authenticated-request concurrency limit.

Each returned allow or deny verdict writes a row to `policy_check_log`. Body or subject validation failures return an HTTP error without an audit row; an invalid operation (for example an undecodable raw transaction) returns an HTTP error and is still audited with reason `decode_error`.

This endpoint predicts privacy and compliance acceptance. It does not predict EVM success, gas use, nonce validity, or later state changes.
