...
2. **JSON-RPC filtering**: Listed DIDs can see event logs from these transactions via `eth_getLogs` and in `eth_getTransactionReceipt` logs. However, `visibleTo` does NOT override receipt participant checks -- a listed DID still receives `null` from `eth_getTransactionReceipt` if they are not a from/to participant.
