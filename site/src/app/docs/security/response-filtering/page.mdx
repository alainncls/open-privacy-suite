...
When `fullTxObjects=true`, the `transactions` array contains full transaction objects (the same shape as `eth_getTransactionByHash` responses). The filter iterates over each transaction and removes entries where neither `from` nor `to` matches a linked address. Block-level fields (`number`, `hash`, `timestamp`, `gasLimit`, etc.) are preserved, but **`logsBloom`, `gasUsed`, and `blobGasUsed` are forcefully zeroed** on every block-shaped response, for every viewer — these aggregate fields would otherwise leak the existence of redacted logs and the total gas weight of other-org activity across the block (RD-873 closed `logsBloom`; RD-929 closed `gasUsed` + `blobGasUsed`).
