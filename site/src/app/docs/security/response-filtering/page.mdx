...
<Callout variant="warning" title="Not covered by response filtering">
  ...
  2. **`eth_call` return data** -- responses are raw ABI-encoded bytes passed through unmodified; address-typed return values are not field-level redacted. Avoid returning sensitive addresses from view functions exposed to unauthorized callers.
