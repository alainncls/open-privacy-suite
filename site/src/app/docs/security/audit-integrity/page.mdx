...
- Create two secrets in AWS Secrets Manager: `<app-db-secret>` and `<admin-db-secret>`.
- Bind the proxy's pod IAM role (IRSA) only to `<app-db-secret>`. The pod can read the app credential at startup; it cannot reach the admin credential.
- Bind a separate IAM role (the operator's break-glass role) to `<admin-db-secret>`. Used via `aws secretsmanager get-secret-value` from a workstation when needed.
