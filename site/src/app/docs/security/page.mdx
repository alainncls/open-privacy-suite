...
Dynamic calls are allowed at deployment because they are validated at runtime via transaction tracing.

### Runtime Transaction Validation

Every transaction is traced using `debug_traceCall` before being forwarded to the node:
