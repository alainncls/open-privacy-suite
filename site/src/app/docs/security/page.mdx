...
| `eth_getStorageAt` | Raw storage slot access bypasses all contract privacy controls |
...
- When `ENABLE_RUNTIME_TRACING=true`, CREATE/CREATE2 opcodes and dynamic calls are allowed — they are validated at runtime via `debug_traceCall`. Child contracts from runtime CREATE/CREATE2 are auto-registered to the deployer's org.
...
|--------|-------------------|
| `eth_getStorageAt` | Permanently blocked (global blocklist) |
...
- **All users** (including deployers and admins) require an explicit `ContractGrant` linking their group to the contract
- When a contract is deployed, the proxy automatically creates grants for the deployer's groups (`CreateDeployerAutoGrants`). Admin can revoke or reassign these grants at any time.
- Unregistered contracts (no registered owner) are denied — all deployed contracts are auto-registered to the deployer's org at deploy time
...
`eth_getStorageAt` is **globally blocked** (raw storage access bypasses all contract-level privacy controls).
...
4. On successful mining: finalizes the pre-registration as a full `Contract` record and **auto-creates `ContractGrant`s** for all the deployer's groups in the org
5. On revert or failure: deletes the pre-registration

This ensures that from the moment the transaction is forwarded, the freshly deployed address belongs to the deployer's org. The auto-grants give the deployer explicit access via their group's claims. Admin can later revoke the deployer's access or reassign the contract to different groups (e.g., a stablecoin-admins group instead of the engineering group).
...
- [ ] Set `CORS_ALLOWED_ORIGINS` to the block explorer's origin (e.g. `https://explorer.yourdomain.com`) — required for the block explorer to call `/eth/link/*` and `/eth/addresses` from the browser
