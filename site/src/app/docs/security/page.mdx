...
|--------|-------------------|
| `eth_getStorageAt` | Tiered access: admin (all slots), read (EIP-1967 proxy slots only), no claim (blocked) |
...
The proxy uses two filtering modes, selected per-contract based on whether event rules are configured:

| Mode | When active | Behavior |
|------|------------|----------|
| **Default (address-based)** | No event rules configured (`event_rules: null`) | Log visible only if user's linked ETH address appears in any topic (zero-padded address match) |
| **Allowlist (event rules)** | Event rules configured (`event_rules: [...]`) | Only logs whose `topic0` matches an allowed event rule pass; optional `param_rules` with `"self"` constraints further restrict by requiring the user's address in specific parameter positions |

An **empty event rules array** (`event_rules: []`) means "deny all events" -- no logs pass regardless of their content.
...
<Callout variant="info" title="Claim hierarchy">
  The `admin` claim implies `read`, `write`, `deploy`, and `upgrade`. Users with `deploy`, `write`, or `read` claims alone do NOT get the admin log bypass -- they are subject to normal event filtering.
</Callout>
...
`eth_getStorageAt` uses **tiered access based on claims** (admin: all slots, read: EIP-1967 proxy slots only, no claim: blocked). See [Response Filtering](/docs/security/response-filtering#eth_getstorageat----tiered-access) for details.
...
Anonymous requests are subject to strict rate limits: **10 requests per second** and **1,000 requests per day**. Unauthenticated requests that target methods outside the claim-free list receive HTTP 401 (not 403), signaling that the client should authenticate rather than that the user lacks permission.
...
The privacy-proxy has three categories of endpoints with different network requirements:
...
All GitHub Actions used in CI/CD workflows are pinned to full commit SHAs rather than mutable tags (e.g., `actions/checkout@<sha>` instead of `actions/checkout@v4`). This prevents a compromised or hijacked upstream action from injecting malicious code into the build pipeline.

Dependabot is configured to monitor GitHub Actions dependencies weekly and open pull requests when new versions are available. This was motivated by the Trivy supply chain attack (March 2026), where a widely-used action tag was force-pushed with malicious content -- SHA pinning would have prevented the compromise from propagating.
...
- [ ] Set `ENVIRONMENT=production` and `GIN_MODE=release`
