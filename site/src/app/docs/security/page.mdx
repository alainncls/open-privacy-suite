...
Authenticated users can also call this same set of org-free metadata methods on `/rpc` without an `org_id` in the path. KYC and ban gates still apply.
...
Unauthenticated requests that target methods outside the claim-free list receive HTTP 401 (not 403), signaling that the client should authenticate rather than that the user lacks permission. Rate limiting for anonymous or high-volume traffic should be enforced upstream (reverse proxy / WAF); the proxy itself enforces method-level limits on expensive trace methods (`debug_trace*`).
