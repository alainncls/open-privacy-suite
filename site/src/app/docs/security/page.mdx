...
not duplicated inside privacy-proxy. Privacy-proxy resolves the caller's group
...
Privacy-proxy separately limits **concurrent in-flight work** because JWT, RBAC,
...
between privacy-proxy and the upstream service that owns node capacity.
