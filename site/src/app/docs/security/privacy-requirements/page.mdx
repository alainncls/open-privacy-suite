...
|---|---|---|---|---|
| `GET /tx/:hash` — `from` / `to` | Real address | `Address-XXXX` | `[PRIVATE]` | `[PRIVATE]` or row dropped |
| `GET /block/:n` — embedded txs | Real address | `Address-XXXX` | `[PRIVATE]` | Row dropped (participant check) |
| `GET /address/:addr` — tx list | Real address | `Address-XXXX` (rows for the granted address only) | `[PRIVATE]` (timing-only audit) | 404 / 403 |
| Token transfer rows (`/transfers`) | Real address | `Address-XXXX` | `[PRIVATE]` | `[PRIVATE]` or row dropped |
...
| Granted-address page (`/grant/:grantId/:addressId`) | Real address | `Address-XXXX` header | `[PRIVATE]` header | 404 |
