...
cross-surface coherence bug to date (RD-1009 and follow-ups), so they are
...
| Contract deployment by Hidden deployer | drop | keep + `[PRIVATE]` deployer, contract address visible, value preserved |
...
skip the transfer-participant union (RD-1009), the `visibleTo` shares,
and the admin-flag wiring — every cross-surface coherence invariant
breaks. PR review must reject hand-rolled opts.

### Coherence invariants

The following invariants are pinned by integration tests
(`internal/server/build_visibility_filter_rd1009_test.go`,
`internal/server/explorer_coherence_e2e_test.go`,
`internal/explorer/redactor_cross_redactor_rd1009_test.go`,
`internal/explorer/redactor_internal_txs_rd1009_test.go`). Failing
any one of them is a privacy or UX regression:

1. **`/transactions` ⊇ `/transfers`** (RD-1009 commit 1).
   ...
   Pre-RD-1009 the two drop predicates evaluated `bothHidden` on
   different address sets — `/transactions` on tx `from`/`to`,
   `/transfers` on transfer `from`/`to` — and disagreed.
2. **List ↔ by-hash agree on row survival** (RD-1009 commit 2).
...
4. **Field-rendering rules are independent of row-survival.**
   ...
   does. Mixing the two axes in one drop predicate is the bug
   class RD-1009 named.
5. **Disclosure-grant rows survive regardless of counterparty privacy.**
   ...
   Pinned by `internal/explorer/redactor_disclosure_grant_matrix_test.go`
   and `internal/server/explorer_grant_byhash_test.go`.
