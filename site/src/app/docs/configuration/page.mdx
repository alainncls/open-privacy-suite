...
<EnvTable vars={[
  ...
  { name: "ETH_CALL_TRACE_TIMEOUT", default: "5s", description: "Per-call timeout for the eth_call trace. Distinct from the 30s send-side trace timeout so a slow upstream cannot fill the per-JWT concurrency quota on the read path." },
]} />
