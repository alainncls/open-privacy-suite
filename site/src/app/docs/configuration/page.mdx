...
**`AUDIT_ADMIN_DATABASE_URL` must own the audit schema, not just hold grants on it.** It runs the migrations, and migrations need DDL rights that `GRANT ALL PRIVILEGES` does not confer. This matters when you follow the path above — start on the derived DSNs, then switch to the dedicated roles — because the derived DSN's role created the audit tables and stays their owner. Transfer them when you switch, as the current owner or a superuser:

```sql
-- in the audit database, as the role the derived DSN connected as
REASSIGN OWNED BY <derived_role> TO privacy_proxy_admin;
GRANT CREATE ON SCHEMA public TO privacy_proxy_admin;
```

If you skip this, the proxy refuses to start and names the table, the current owner and the statement to run. Existing grants (including the `privacy_proxy_app` append-only allowlist) survive the transfer untouched.
