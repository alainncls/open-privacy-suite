...
REVOKE privacy_proxy_admin FROM <derived_role>;
```

Existing grants survive the transfer, including the `privacy_proxy_app` append-only allowlist on `access_logs`. Skip the transfer and the proxy refuses to start, naming the table, its owner and the statement to run.
