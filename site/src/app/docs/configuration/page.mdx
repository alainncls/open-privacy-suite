...
You can isolate the access log in its own PostgreSQL so the running proxy can **append** access-log rows but cannot rewrite or delete history. Set both DSNs (they are secrets — keep them in the environment / your secrets manager, not the config file):

<EnvTable vars={[
  { name: "AUDIT_DATABASE_URL", default: "(empty)", description: "Runtime connection string for the separate access-log database. Must connect as the RESTRICTED database role — the one that has INSERT + SELECT on access_logs but not UPDATE/DELETE. Empty = access_logs stay in DATABASE_URL (unchanged)." },
  { name: "AUDIT_ADMIN_DATABASE_URL", default: "(empty)", description: "Admin/owner connection string for the same database. The proxy uses it to run the audit-database migrations at startup (schema + the append-only tightening) and to run retention pruning (which the restricted runtime role cannot do). Pair it with AUDIT_DATABASE_URL." },
]} />

When both are set, the proxy migrates the audit database at startup, then reads and writes all access-log data there; the main `DATABASE_URL` no longer holds access logs. Leave both empty for the default single-database deployment.

**Deployment dependency — the restricted role is what makes this work.** The append-only guarantee comes from the *database role* the runtime DSN connects as, not from the proxy. `AUDIT_DATABASE_URL` **must** connect as the restricted role (the one migration `058` calls `privacy_proxy_app`, further tightened to INSERT-only on the audit database). If `AUDIT_DATABASE_URL` connects as the database owner instead, the proxy still works but the seal does **nothing** — the owner can update and delete rows. `AUDIT_ADMIN_DATABASE_URL` is the owner DSN and is used only for migrations and retention pruning.

If the audit database is unreachable, access-log writes fail the same way they do today (best-effort on the request path); the request itself is not blocked. A fail-closed mode is not yet available.
...
<EnvTable vars={[
  ...
  { name: "BILLIONS_RPC_URL", default: "https://billions-rpc.example.com", description: "RPC endpoint for the Billions identity chain (chainID 45056). Lets users sign in with the Billions app, not just Privado ID." },
...
|---------|---------|---------|
| `INDEXER_VERSION` | `0.2.2` (dev) / `latest` (prod) | Tag pulled from `gatewayfm/chain-indexer` (dev, dockerhub) or `ghcr.io/gateway-fm/chain-indexer` (prod). Verify any new pin with `docker manifest inspect` before changing — the dev default has no `v` prefix because the dockerhub tag doesn't carry one. |
...
- Increments Prometheus counter `privacy_proxy_audit_chain_integrity_violations_total{chain,reason}`.
