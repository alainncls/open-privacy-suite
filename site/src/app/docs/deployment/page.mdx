...
This guide is for **users deploying their own contracts through the proxy**. If you're an operator rolling out the Open Privacy Suite stack itself (compose manifests, secrets, trust zones), see [Operator Deployment](/docs/operator-deployment) instead.

The proxy validates all transactions at runtime using `debug_traceCall`, so deploying is nearly identical to deploying on any public chain.
...
Access tokens expire after 5 minutes (production; dev/mockauth builds use 30 minutes). To refresh:
...
| `403 transaction denied: runtime contract creation requires the deploy claim` | User lacks deploy permission | Ask org admin to grant `deploy` claim |
| `403 transaction denied: cross-org access not permitted` | A transaction's runtime trace enters another org's contract | Only interact with your org's contracts |
| `404 method not found` | Method not in user's allowlist (RBAC denials are masked as an opaque 404, not a 403) | Check group permissions |
...
The proxy enforces HTTPS for the `FRONTEND_URL` in production (`ENVIRONMENT=production`). However, **private network addresses are exempt** from this requirement -- HTTP is allowed when the URL points at:
...
The backend starts successfully even if the explorer database is unavailable. It logs a warning and continues serving all non-explorer endpoints, retrying the connection in the background. Explorer API endpoints return errors until the connection is established.

This supports deployment scenarios where the explorer database is provisioned after the proxy.
