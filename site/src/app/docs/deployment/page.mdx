...
export const metadata = {
  ...
  description:
    "How to deploy smart contracts through the Privacy Proxy using runtime tracing.",
};

# Contract Deployment

This guide describes how to deploy smart contracts through the Privacy Proxy. The proxy validates all transactions at runtime using `debug_traceCall`, so deploying is nearly identical to deploying on any public chain.
...
Open the Privacy Proxy web UI (e.g., `http://localhost:5173`), authenticate with your Privado ID wallet (scan QR code), then go to **Settings** > **Developer** > **Copy Access Token**.
...
2. Address is immediately pre-registered to the deployer's org (closes the cross-org access window).
