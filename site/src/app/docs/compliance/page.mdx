...
<Callout variant="warning" title="Compliance is disabled by default">
...
- **Cluster-wide default** — set the `COMPLIANCE_DEFAULT_MODE` environment variable (`enforce` | `monitor`, default `enforce`). A per-org setting overrides it.
...
This layer converts the transfer amount to the organization's configured [currency](#currency) and compares it against the applicable threshold. Only transfers at or above the threshold proceed to Layer 3.
...
The amount — in the organization's configured [currency](#currency) — at or above which a transfer requires travel rule documentation.
...
- The transfer amount is converted to the organization's currency using the token prices configured for that currency (see [Token Prices](#token-prices)). If a token has no price in the organization's currency, the transfer **fails closed**.
...
- The application database role holds only `SELECT, INSERT` on the compliance log — no `UPDATE`/`DELETE` grant — so the running proxy cannot rewrite or delete a decision (enforced by GRANT/role separation, not database triggers). Retention pruning runs under a separate privileged role.
...
- No `UPDATE`/`DELETE` grant on the log for the application role (GRANT/role separation, not database triggers)
