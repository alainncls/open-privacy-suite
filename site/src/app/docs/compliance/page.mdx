...
Since the Open Privacy Suite operates on a private network without external oracle access, token prices must be manually configured by administrators.

- Price expressed in USD per token unit (e.g., $2,000 per ETH)
- Prices can be updated at any time; changes apply immediately to new transfers
- Missing price for any token results in transfer denial (fail-closed)
